...
public class MatchingEngineTests
{
    ...
    [Fact]
    public async Task OrderBookManager_ReturnsSameBookForSameInstrument()
    {
        var manager = new OrderBookManager();

        var instrumentId = Guid.NewGuid();

        var first = manager.GetOrCreate(instrumentId);
        var second = manager.GetOrCreate(instrumentId);

        Assert.Same(first, second);
    }
    [Fact]
    public void OrderBookManager_CreatesDifferentBooksForDifferentInstruments()
    {
        var manager = new OrderBookManager();

        var sberBook = manager.GetOrCreate(Guid.NewGuid());
        var gazpBook = manager.GetOrCreate(Guid.NewGuid());

        Assert.NotSame(sberBook, gazpBook);
    }
    [Fact]
    public void OrderBookManager_Load_RebuildsBook()
    {
        var instrumentId = Guid.NewGuid();
        ...
        Assert.Same(book, loadedBook);

        var bids = loadedBook!.Bids.ToArray();

        Assert.Equal(251m, bids[0].Price);
        Assert.Equal(250m, bids[1].Price);
    }
}